...
class MiniGridEnv(gym.Env):
    ...
    def user_defined_place_obj(self,
        obj, x, y
    ):
        """
        ...
        assert x < top[0] + size[0]
        assert y < top[1] + size[1]
        pos = np.array((x,y))

        # Don't place the object on top of another object
        # Don't place the object where the agent is
        #pdb.set_trace()
        if self.grid.get(*pos) == None and not np.array_equal(pos, self.start_pos):
    ...
    def gen_full_obs_without_agent(self):
        grid = self.grid.slice(0, 0, self.grid.width, self.grid.height)
        vis_mask = np.ones(shape=(grid.width, grid.height), dtype=np.bool)
        image = grid.encode(vis_mask)
        return image
    ...
    def render(self, mode='human', close=False, global_view=False):
        """
        Render the whole-grid human view

        Args:
            global_view (Bool):     If true, will not render agent observable mask. NOTE: added by Yiye
        """
