...
class GoNextToSubgoal(Subgoal):
    ...
    def replan_before_action(self):
        ...
        if isinstance(self.datum, ObjDesc):
            ...
            #if (self.datum.type == 'key'):
            #    self.bot.key_pos = list(target_pos)
            if not target_pos:
...
class Bot:
    ...
    def get_goal_state(self, action_taken=None):
        goal = self.stack[-1].datum
        return goal.obj_poss

    def get_key_state(self):
        return self.key_pos

    def get_possible_last_state(self, goal_pos):
        grid, vis_mask = self.mission.gen_full_grid()
        near_poses = [[-1,0],[1,0],[0,-1],[0,1]]
        for pos in near_poses:
            cell = grid.get(goal_pos[0]+pos[0], goal_pos[1]+pos[1])
            if (cell is None):
                if (pos == [-1,0]):
                    dir = 0
                elif (pos == [1,0]):
                    ...
                    dir = 3
                return [goal_pos[0]+pos[0], goal_pos[1]+pos[1], dir]
        
    def check_door_state(self, door_pos):
        grid, vis_mask = self.mission.gen_full_grid()
        cell = grid.get(door_pos[0], door_pos[1])
        return cell.is_locked


    def _process_full_obs(self):
        ...
        for vis_j in range(0, grid.height):
            for vis_i in range(0, grid.width):
                '''
                ...
                if abs_j < 0 or abs_j >= self.vis_mask.shape[1]:
                    continue
                '''
                self.vis_mask[vis_i, vis_j] = True


    def full_obs_replan(self, action_taken=None):
        ...
        #if (suggested_action == self.mission.actions.done):
        #    break
